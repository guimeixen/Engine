...
namespace Engine
{
	...
	Renderer *Renderer::Create(GLFWwindow *window, GraphicsAPI api, unsigned int width, unsigned int height, unsigned int monitorWidth, unsigned int monitorHeight)
	{
	...
	void Renderer::CreateRenderQueues(unsigned int passAndFrustumCount, unsigned int *passIds, const std::vector<VisibilityIndices> &visibility, RenderQueue *outQueues)
	{
		...
		for (unsigned int j = 0; j < passAndFrustumCount; j++)
		{
			...
			for (size_t k = 0; k < renderQueueGenerators.size(); k++)
			{
				...
				std::memcpy(outQueues[j].data() + offset, queues[k][j].data(), sizeof(RenderItem) * queueSize);
				offset += queueSize;
			}
		}
	}
