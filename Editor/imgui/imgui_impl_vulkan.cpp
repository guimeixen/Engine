...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    VkDescriptorSet curSet = g_DescriptorSet;
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
                {
                    ...
                    // We keep track of the previously bound desc set and the one we want to bind to avoid redundant binds
                    if (curSet != lastSet)
                    {
                        if ((Engine::Texture*)pcmd->TextureId == gameViewTexture)
                        {
...
void ImGUI_ImplVulkan_SetGameViewTexture(Engine::Texture* texture, bool updateDescSet)
{
    ...
    if (gameViewDescSet == VK_NULL_HANDLE)
    {
        ...
        check_vk_result(vkAllocateDescriptorSets(v->Device, &setAllocInfo, &gameViewDescSet));
...
void ImGUI_ImplVulkan_CreateOrResizeBuffers(VkDeviceSize vertexSize, VkDeviceSize indexSize)
{
    ...
    for (uint32_t n = 0; n < g_MainWindowRenderBuffers.Count; n++)
    {
        ImGui_ImplVulkanH_FrameRenderBuffers* fd = &g_MainWindowRenderBuffers.FrameRenderBuffers[n];
...
bool    ImGui_ImplVulkan_Init(Engine::Renderer* renderer, ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass)
{
