...
void ImGUI_ImplVulkan_SetGameViewTexture(Engine::Texture *texture, bool updateDescSet)
{
	...
	if (gameViewDescSet == VK_NULL_HANDLE)
	{
		...
		setAllocInfo.descriptorPool = g_DescriptorPool;
		setAllocInfo.descriptorSetCount = 1;
		setAllocInfo.pSetLayouts = &g_DescriptorSetLayout;

		check_vk_result(vkAllocateDescriptorSets(g_Device, &setAllocInfo, &gameViewDescSet));
...
bool ImGui_ImplVulkan_CreateDeviceObjects(Engine::Renderer *renderer)
{
    VkResult err;
