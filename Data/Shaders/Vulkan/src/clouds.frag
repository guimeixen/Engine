...
/*layout(set = 1, binding = 0) uniform sampler3D baseNoiseTexture;
layout(set = 1, binding = 1) uniform sampler3D highFreqNoiseTexture;
layout(set = 1, binding = 2) uniform sampler2D weatherTexture;*/
...
vec4 clouds(vec3 dir)
{
	...
	for (float i = 0.0; i < steps; i += stepMult)
	{
		...
		{		
			...
			//vec3 ambient = mix(ambientBottomColor.rgb, ambientTopColor.rgb, heightFraction) * ambientMult;
	...
	vec3 col = highClouds * HG * dirLightColor.xyz * directLightMult;
...
void main()
{
	...
	color =  clouds(dir);
//	color = texture(sceneDepth,uv);
	//color = vec4(dir, 1.0);
	//color = texture(weatherTexture, uv);
